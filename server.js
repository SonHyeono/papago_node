...
app.post("/detectLangs", (req, res) => {
    ...
    //JSON을 object로 파싱해줘야하는데 javascript는 알아서 해준다
    ...
    request.post(options, (error, response, body) => { 
        if(!error && response.statusCode == 200){
            ...
            res.redirect(`translate?lang=${parseBody['langCode']}&targetLanguage=${targetLanguage}&query=${query}`); // 쿼리 스트링(query string) -> get요청
